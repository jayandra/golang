...
func main() {
	...
	for !exit_loop {
		...
		go func() {
			...
			resultChannel <- AreaComputer{shape}.ComputeArea()
		}()

		fmt.Printf("In main.........\n")

		x := <-resultChannel
		fmt.Printf("The Area is: %.2f", x)

	}
}

//  Pass by value vs pass by reference had no effect on the execution
//  The source of failure would be #ComputeArea beign called before subroutine sets the shape to compute the area upon.
//
